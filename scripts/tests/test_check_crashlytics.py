...
def test_constants():
    assert cc.PROJECT_ID == "answerguard-486213"
    assert cc.PACKAGE == "com.igorganapolsky.answerguard"
...
def test_check_bigquery_export_returns_table_names():
    with patch.object(cc.urllib.request, "urlopen") as mock_urlopen:
        ...
        response_data = {
            "tables": [
                {"tableReference": {"tableId": "com_iganapolsky_answerguard"}},
                {"tableReference": {"tableId": "com_iganapolsky_answerguard_debug"}},
            ]
        }
        mock_resp = MagicMock()
        mock_resp.read.return_value = json.dumps(response_data).encode()
        ...
        result = cc.check_bigquery_export("fake_token")
        assert result == ["com_iganapolsky_answerguard", "com_iganapolsky_answerguard_debug"]
