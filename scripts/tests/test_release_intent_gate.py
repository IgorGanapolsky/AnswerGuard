...
def test_release_branch_ios_without_confirm_blocks():
    block, msg = should_block("refs/heads/release/v1.2.7", "ios", "false")
    assert block is True
    assert "skips Android" in msg


def test_release_branch_ios_with_confirm_ok():
    block, _ = should_block("refs/heads/release/v1.2.7", "ios", "true")
    assert block is False


def test_develop_ios_ok_without_confirm():
    block, _ = should_block("refs/heads/develop", "ios", "false")
    assert block is False
