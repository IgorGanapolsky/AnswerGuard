...
def create_title_experiment(repo_root: Path) -> Dict[str, Any]:
    ...
    base_title = "AnswerGuard"
    variants = [
        {"id": "control", "title": base_title},
        {"id": "variant_a", "title": "AnswerGuard - HIIT & Drills"},
...
def ensure_localized_metadata(repo_root: Path) -> Dict[str, Any]:
    ...
    for locale, lang_name in TOP_MARKETS.items():
        # Android metadata
        android_dir = repo_root / f"native-android/fastlane/metadata/android/{locale}"
        ...
        # iOS metadata
        ios_dir = repo_root / f"native-ios/fastlane/metadata/{locale}"
