...
DEFAULT_IOS_APP_ID = "6766520485"
DEFAULT_ANDROID_PACKAGE = "com.igorganapolsky.answerguard"
DEFAULT_COUNTRY = "US"
DEFAULT_TIMEOUT = 900
...
def read_ios_version(repo_root: Path = ROOT) -> str:
    project = repo_root / "native-ios/AnswerGuard.xcodeproj/project.pbxproj"
...
def verify_app_store_public_version(
    ...
    try:
        response = requests.get(
            url,
            timeout=30,
            headers={"User-Agent": "AnswerGuard-Store-Version-Readback/1.0"},
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Stop polling when observed versions are stable but still mismatched "
            "(public storefront propagation lag; advisory CI readback)."
        ),
    )
