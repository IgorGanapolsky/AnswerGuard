...
ASC_BUNDLE_ID = "com.igorganapolsky.answerguard"
...
def check_public_store_tier2(expected_version: str, *, timeout: int) -> list[CheckResult]:
    ...
    ios = verify_app_store_public_version(DEFAULT_IOS_APP_ID, expected_version)
    play = verify_play_public_version(DEFAULT_ANDROID_PACKAGE, expected_version)
...
def check_play_iap_catalog() -> CheckResult:
    if not _has_play_credentials():
        return CheckResult(
            ...
            False,
            "Play Monetization API: answerguard_family + answerguard_business ACTIVE; answerguard_pro ACTIVE.",
...
def check_asc_version_state(version: str) -> CheckResult:
    if not _has_asc_credentials():
        return CheckResult(
            ...
            f"PYTHONPATH=. python3 scripts/asc_watch_status.py --version {version}",
            {"reason": "missing APPSTORE_KEY_ID / ISSUER_ID / PRIVATE_KEY"},
        )
    cmd = [
        sys.executable,
        str(SCRIPTS / "asc_watch_status.py"),
        "--bundle-id",
        ASC_BUNDLE_ID,
        "--version",
        version,
