...
Usage: ci_firebase_apptesting_execute.sh [--apk PATH] [--test-dir DIR] [--test-devices SPEC] [--non-blocking] [--debug]
  ...
  --apk             Path to APK. Omit to use latest App Distribution release for --app.
  --test-dir        Directory where agent YAML suites live.
  --test-devices    Semicolon-separated Test Lab device specs.
  --non-blocking    Pass --test-non-blocking to firebase CLI.
  --debug           Pass --debug to firebase CLI.

Environment:
  FIREBASE_ANDROID_APP_ID          Required Firebase Android app id.
  FIREBASE_SERVICE_ACCOUNT_JSON    Service account JSON body.
  GOOGLE_APPLICATION_CREDENTIALS   Optional service account JSON file path.
USAGE
}
...
echo "Running Firebase App Testing agent"
"${FB[@]}"
