...
adb shell mkdir -p /sdcard/phoneclaw/answerguard 2>/dev/null || true

# Push ClawScript files
echo -e "${CYAN}Pushing ClawScripts to device...${RESET}"
adb push "$SCRIPT_DIR/visual-alarm-notification.js" /sdcard/phoneclaw/answerguard/
adb push "$SCRIPT_DIR/visual-lockscreen-alarm.js" /sdcard/phoneclaw/answerguard/

echo -e "${GREEN}Scripts pushed to /sdcard/phoneclaw/answerguard/${RESET}"
echo ""
echo -e "${BOLD}To run visual tests:${RESET}"
echo "  1. Open PhoneClaw app on the device"
echo "  2. Load script from /sdcard/phoneclaw/answerguard/"
echo "  3. Start AnswerGuard alarm (use: make device-tests-adb to trigger)"
