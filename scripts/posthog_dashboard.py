#!/usr/bin/env python3
"""Comprehensive PostHog analytics dashboard for AnswerGuard.

Queries PostHog HogQL API to build a full-funnel dashboard covering:
- Install funnel with conversion rates (7d and 30d)
- Engagement metrics (WSBU, DAU, avg spam_call_blocked)
- Feature popularity breakdowns
- Revenue metrics
- Retention (daily spam_call_blocked, new vs returning)

AnswerGuard is a call-screening app: the core "value delivered" event is
`spam_call_blocked` (the equivalent of `timer_completed` in the template).
Event names come from
native-android/app/src/main/java/com/igorganapolsky/answerguard/analytics/AnalyticsService.kt.
...
def _funnel_section(
    ...
    AnswerGuard onboarding funnel:
      install -> first_open -> first_protection_enabled
              -> first_spam_blocked -> paywall_viewed -> paywall_purchase_success
    """
    f = PRAGMATIC_LIVE
    funnel_events = [
        ("Application Installed", "application_installed"),
        ("first_open", "first_open"),
        ("first_protection_enabled", "first_protection_enabled"),
        ("first_spam_blocked", "first_spam_blocked"),
...
def _engagement_section(
    ...
    """WSBU, DAU, avg spam_call_blocked per user per week.

    WSBU = Weekly Spam-Blocking Users: distinct users with >=3 spam_call_blocked
    events in the trailing 7 days. This is AnswerGuard's North-Star engagement
    metric (the call-screening analog of the template's WQTU/timer_completed).
    """
    f = PRAGMATIC_LIVE

    # WSBU: distinct users with >=3 spam_call_blocked in trailing 7 days
    wsbu = _scalar(
        f"SELECT count() FROM ("
        f"  SELECT person_id, count() AS c FROM events "
        f"  WHERE event = 'spam_call_blocked' "
    ...
    # Average spam_call_blocked per user per week (over the trailing 7d window)
    avg_blocked_per_user_week = _scalar_float(
        f"SELECT avg(c) FROM ("
        f"  SELECT person_id, count() AS c FROM events "
        f"  WHERE event = 'spam_call_blocked' "
        f"  AND timestamp > now() - interval 7 day AND {f} "
    ...
    # Total call_screened events in the window (calls that hit the screening pipeline)
    win = f"{days} day"
    calls_screened = _scalar(
        f"SELECT count() FROM events "
        f"WHERE event = 'call_screened' "
        f"AND timestamp > now() - interval {win} AND {f}",
        api_key, project_id, errors,
    )
    ...
    return {
        ...
        "dau": dau,
        "wau_7d": wau,
        "avg_spam_call_blocked_per_user_7d": avg_blocked_per_user_week,
        "calls_screened_window": calls_screened,
        "window_days": days,
    }
...
def _feature_popularity_section(
    ...
    """Feature popularity breakdowns for AnswerGuard call-screening flows."""
    f = PRAGMATIC_LIVE
    win = f"{days} day"

    # spam_call_blocked vs spam_call_silenced volume comparison
    block_vs_silence = _rows(
        f"SELECT event, count(), count(DISTINCT person_id) FROM events "
        f"WHERE event IN ('spam_call_blocked', 'spam_call_silenced', 'call_screened') "
        f"AND timestamp > now() - interval {win} AND {f} "
        f"GROUP BY 1 ORDER BY 2 DESC",
        api_key, project_id, errors,
    )

    # call_screening_enabled by entry_point
    screening_enabled = _rows(
        f"SELECT coalesce(toString(properties.entry_point), '(unknown)'), "
        f"count(), count(DISTINCT person_id) FROM events "
        f"WHERE event = 'call_screening_enabled' "
        f"AND timestamp > now() - interval {win} AND {f} "
    ...
    # Review prompt funnel: review_prompt_requested -> write_review_tapped
    review_funnel = _rows(
        f"SELECT event, count(), count(DISTINCT person_id) FROM events "
        f"WHERE event IN ('review_prompt_requested', 'write_review_tapped') "
        f"AND timestamp > now() - interval {win} AND {f} "
        f"GROUP BY 1 ORDER BY 2 DESC",
        api_key, project_id, errors,
    )
    ...
    return {
        "screening_outcomes": [
            {"event": r[0], "events": r[1], "distinct_users": r[2]}
            for r in block_vs_silence
        ],
        "call_screening_enabled_by_entry_point": [
            {"entry_point": r[0], "events": r[1], "distinct_users": r[2]}
            for r in screening_enabled
        ],
        "settings_changed": [
            {"setting": r[0], "events": r[1], "distinct_users": r[2]}
            for r in settings_changed
        ],
        "review_prompt_funnel": [
            {"event": r[0], "events": r[1], "distinct_users": r[2]}
            for r in review_funnel
        ],
    }
...
def _retention_section(
    ...
    """Daily spam_call_blocked over last 30 days + new vs returning users."""
    f = PRAGMATIC_LIVE

    # Daily spam_call_blocked for last 30 days
    daily_blocked = _rows(
    ...
    return {
        "daily_spam_call_blocked_30d": [
            {"date": str(r[0]), "events": r[1], "distinct_users": r[2]}
            for r in daily_blocked
        ],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Comprehensive PostHog analytics dashboard for AnswerGuard"
    )
    parser.add_argument(
    ...
    if json_stdout:
        ...
        print("=" * 60)
        print("  ANSWERGUARD POSTHOG ANALYTICS DASHBOARD")
        print("=" * 60)
        ...
        print(f"  WSBU (7d):  {eng.get('wsbu_7d')}")
        print(f"  DAU:        {eng.get('dau')}")
        print(f"  WAU (7d):   {eng.get('wau_7d')}")
        print(f"  Avg spam_call_blocked/user/week: {eng.get('avg_spam_call_blocked_per_user_7d')}")
        print(f"  Calls screened (window):         {eng.get('calls_screened_window')}")
        print()
