...
AnswerGuard is a call-screening / spam blocker. Data Safety declaration MUST
match the runtime permissions in AndroidManifest.xml:

  Permissions requested by AnswerGuard:
    - android.permission.INTERNET                         (analytics, future spam intelligence updates)
    - com.android.vending.BILLING                         (Play Billing for Pro upgrade)
    - android.permission.READ_CONTACTS                    (avoid blocking saved contacts; on-device only)
    - android.permission.BIND_SCREENING_SERVICE           (CallScreeningService binding; not user data)

  Permission EXPLICITLY removed (do not declare):
    - com.google.android.gms.permission.AD_ID             (Firebase/PostHog stripped via tools:node="remove")

  Data collected/handled (must reflect on Data Safety form):
    - Contacts (phone numbers + display names): processed ON-DEVICE only to
      whitelist saved contacts during call screening. Never uploaded, never shared.
    - Purchase tokens: collected by Google Play Billing for entitlement
      verification. Required for app functionality.
    - Crash logs (Firebase Crashlytics): collected for app stability.
    - Anonymous product analytics (PostHog): no advertising ID, no user
      identifiers; only event counts and feature usage.
