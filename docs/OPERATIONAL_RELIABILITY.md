...
- Use `store-release-watcher.yml` (cron every 6h) for asynchronous Tier 2 monitoring.
...
- **Full `connectedDebugAndroidTest` gate:** `android-device-test-gate` runs `scripts/device-tests/ci-connected-all.sh` — two instrumentation classes (`RoleOnboardingTest`, `RecentActivityTest`), one Gradle invocation per class, `am force-stop` only between classes (never in `@AfterClass` on the last class of a run).
