...
final class TimerConfigProClampingTests: XCTestCase {
    ...
    func testExpiredProUser_maxSecondsAboveFreeLimit_isClamped() {
        let proConfig = AnswerGuard.TimerConfig(
            minSeconds: 0,
            maxSeconds: 3600,
            alarmDuration: 10,
            hiddenMode: false,
        ...
        XCTAssertEqual(clamped.maxSeconds, AnswerGuard.TimerConfig.maxSecondsFree,
    ...
    func testExpiredProUser_proSoundType_isResetToIntense() {
        ...
        XCTAssertEqual(clamped.soundType, AnswerGuard.SoundType.intense,
    ...
    func testFreeUserConfig_withinFreeLimits_isUnchanged() {
        let freeConfig = AnswerGuard.TimerConfig(
            minSeconds: 30,
            maxSeconds: 300,
            alarmDuration: 10,
            hiddenMode: false,
    ...
    func testExpiredProUser_allProSoundTypes_areResetToIntense() {
        let proSounds: [AnswerGuard.SoundType] = [.klaxon, .whistle, .buzzer, .gong, .airhorn, .drumRoll, .siren, .bell]
