...
final class TimerConfigTests: XCTestCase {
    ...
    func testDefaultConfigHasValidRange() {
        let config = AnswerGuard.TimerConfig.default
    ...
    func testConfigAcceptsValidRange() {
        let config = AnswerGuard.TimerConfig(
            minSeconds: 60,
            maxSeconds: 300,
            alarmDuration: 10,
            hiddenMode: false,
    ...
    func testConfigDurationConversion() {
        let config = AnswerGuard.TimerConfig(minSeconds: 90, maxSeconds: 300)
    ...
    func testConfigDecodingSupportsLegacyKeysAndLooseSoundNames() throws {
        ...
        let decoded = try JSONDecoder().decode(AnswerGuard.TimerConfig.self, from: payload)

        XCTAssertEqual(decoded.minSeconds, 0)
        XCTAssertEqual(decoded.maxSeconds, AnswerGuard.TimerConfig.maxSecondsPro)
    ...
    func testConfigDecodingFallsBackToDefaultsWhenFieldsMissing() throws {
        ...
        let expected = AnswerGuard.TimerConfig(
            minSeconds: 0,
            maxSeconds: 300,
            alarmDuration: 10,
            hiddenMode: false,
    ...
    func testConfigDecodingCapsOversizedMinAtProLimit() throws {
        ...
        XCTAssertEqual(decoded.minSeconds, AnswerGuard.TimerConfig.maxSecondsPro)
        XCTAssertEqual(decoded.maxSeconds, AnswerGuard.TimerConfig.maxSecondsPro)
    }
}
...
final class TimerStateTests: XCTestCase {
    ...
    private let defaultConfig = AnswerGuard.TimerConfig.default
    ...
    func testStateDecodingSupportsLegacyKeysAndStatusNormalization() throws {
        ...
        let decoded = try JSONDecoder().decode(AnswerGuard.TimerState.self, from: payload)
...
final class SoundTypeTests: XCTestCase {
    ...
    func testIntenseNotificationSoundName() {
        XCTAssertEqual(AnswerGuard.SoundType.intense.notificationSoundName, "alarm.mp3")
    }

    func testGentleNotificationSoundName() {
        XCTAssertEqual(AnswerGuard.SoundType.gentle.notificationSoundName, "gentle-chime.mp3")
    }
}
...
final class TimerManagerTests: XCTestCase {
    ...
    func testStartTimerSetsTimerState() async {
        let manager = TimerManager()
        let config = AnswerGuard.TimerConfig(minSeconds: 5, maxSeconds: 10)
    ...
    func testLoopTogglePropagatesToRunningTimer() async {
        ...
        newConfig = AnswerGuard.TimerConfig(
    ...
    func testUpdateConfigDoesNotCrashWhenNoTimerRunning() {
        ...
        let newConfig = AnswerGuard.TimerConfig(
            minSeconds: 1,
            maxSeconds: 300,
            alarmDuration: 5,
            hiddenMode: false,
...
final class LiveActivityTimingLeakTests: XCTestCase {
    ...
    func testLiveActivityUpdateSendsZeroRemainingSeconds() {
        ...
        let config = AnswerGuard.TimerConfig(minSeconds: 30, maxSeconds: 300, alarmDuration: 10)
        let state = AnswerGuard.TimerState(
            config: config,
...
final class TimerManagerSilenceTests: XCTestCase {
    ...
    func testSilenceAlarmSetsIsAlarmSilencedTrue() {
        ...
        let alarmState = AnswerGuard.TimerState(
...
final class StorageServiceTests: XCTestCase {
    ...
    func testSaveAndLoadTimerStatePersistsAcrossAsyncAndSyncAPIs() async {
        let config = AnswerGuard.TimerConfig(minSeconds: 20, maxSeconds: 80, alarmDuration: 10)
    ...
    func testCancelTimerIncludesAbandonReasonAndSource() async {
        ...
        manager._setTimerStateForTesting(
            AnswerGuard.TimerState(
                config: AnswerGuard.TimerConfig.default,
                targetDuration: 300,
