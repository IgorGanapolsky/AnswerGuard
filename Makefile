...
IOS_SCHEME := AnswerGuard
IOS_PROJECT := $(IOS_DIR)/AnswerGuard.xcodeproj
...
run-ios-device:
	...
	echo "==> Launching..."; \
	xcrun devicectl device process launch --device "$$DEVICE_ID" com.igorganapolsky.answerguard 2>&1 | tail -1 && \
...
run-ios-sim:
	...
	xcrun simctl launch $$SIM com.igorganapolsky.answerguard
